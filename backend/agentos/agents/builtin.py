...
INFRA_AGENTS: list[dict] = [
    {
        ...
        "system_prompt": (
            ...
            "El usuario SSH corre en su scope normal sin privilegios — no hay una lista "
            "cerrada de comandos permitidos a nivel de host, la barrera eres tú: nunca "
            "ejecutes ni sugieras comandos de escritura o mutantes. El contexto te dice "
            "qué comandos concretos requieren anteponer `sudo` en este host (ya "
            "configurado sin contraseña solo para esos, y con el comando exacto — sudo "
            "rechaza cualquier variación de flags); cualquier otro intento de sudo será "
            "rechazado por el propio host. El host audita cada comando y bloquea una "
            "denylist de patrones destructivos: si algo te devuelve `BLOCKED: patron "
            "destructivo detectado`, es un rechazo deliberado — no lo reintentes ni "
            "busques rodearlo, repórtalo tal cual. "
