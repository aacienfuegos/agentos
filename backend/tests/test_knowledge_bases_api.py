"""Tests for /api/knowledge-bases CRUD and file browser endpoints."""
...
KB_PAYLOAD = {
    ...
    "instructions": {"cite_verbatim": False, "free_text": ""},
}


def test_list_empty(app_client: TestClient):
    response = app_client.get("/api/knowledge-bases")
...
def test_create(app_client: TestClient, tmp_path: Path):
    ...
    response = app_client.post("/api/knowledge-bases", json=payload)
...
def test_create_auto_path(app_client: TestClient):
    ...
    response = app_client.post("/api/knowledge-bases", json=KB_PAYLOAD)
    assert response.status_code == 201
    data = response.json()
...
def test_get(app_client: TestClient, tmp_path: Path):
    payload = {**KB_PAYLOAD, "knowledge_path": str(tmp_path / "homelab")}
    app_client.post("/api/knowledge-bases", json=payload)
    response = app_client.get("/api/knowledge-bases/homelab")
    assert response.status_code == 200
    assert response.json()["id"] == "homelab"


def test_get_not_found(app_client: TestClient):
    response = app_client.get("/api/knowledge-bases/nonexistent")
...
def test_update(app_client: TestClient, tmp_path: Path):
    ...
    response = app_client.put(
        "/api/knowledge-bases/homelab",
        json={"name": "Homelab Updated"},
    )
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "Homelab Updated"
    assert data["description"] == KB_PAYLOAD["description"]


def test_update_not_found(app_client: TestClient):
    response = app_client.put("/api/knowledge-bases/nonexistent", json={"name": "X"})
    assert response.status_code == 404
...
def test_delete(app_client: TestClient, tmp_path: Path):
    payload = {**KB_PAYLOAD, "knowledge_path": str(tmp_path / "homelab")}
    app_client.post("/api/knowledge-bases", json=payload)
    response = app_client.delete("/api/knowledge-bases/homelab")
    assert response.status_code == 204
    assert app_client.get("/api/knowledge-bases/homelab").status_code == 404
...
def agent_with_dir(app_client: TestClient, tmp_path: Path):
    ...
    payload = {**KB_PAYLOAD, "knowledge_path": str(kpath)}
    app_client.post("/api/knowledge-bases", json=payload)
    return str(kpath)


def test_list_files(app_client: TestClient, agent_with_dir):
    response = app_client.get("/api/knowledge-bases/homelab/files")
    assert response.status_code == 200
...
def test_get_file(app_client: TestClient, agent_with_dir):
    response = app_client.get("/api/knowledge-bases/homelab/files/README.md")
    assert response.status_code == 200
    assert "# Homelab" in response.text


def test_get_file_nested(app_client: TestClient, agent_with_dir):
    response = app_client.get("/api/knowledge-bases/homelab/files/infra/docker.md")
    assert response.status_code == 200
    assert "# Docker" in response.text


def test_get_file_not_found(app_client: TestClient, agent_with_dir):
    response = app_client.get("/api/knowledge-bases/homelab/files/nope.md")
    assert response.status_code == 404


def test_get_file_traversal(app_client: TestClient, agent_with_dir):
    response = app_client.get("/api/knowledge-bases/homelab/files/../../etc/passwd")
...
def test_update_file(app_client: TestClient, agent_with_dir):
    ...
    response = app_client.put(
        "/api/knowledge-bases/homelab/files/README.md",
        content=new_content,
...
def test_update_file_creates_new(app_client: TestClient, agent_with_dir):
    response = app_client.put(
        "/api/knowledge-bases/homelab/files/software/traefik.md",
        content="# Traefik\n",
        headers={"Content-Type": "text/plain"},
    )
    assert response.status_code == 200
...
def test_delete_file(app_client: TestClient, agent_with_dir):
    response = app_client.delete("/api/knowledge-bases/homelab/files/README.md")
    assert response.status_code == 204
    assert app_client.get("/api/knowledge-bases/homelab/files/README.md").status_code == 404


def test_delete_file_not_found(app_client: TestClient, agent_with_dir):
    response = app_client.delete("/api/knowledge-bases/homelab/files/nope.md")
    assert response.status_code == 404
...
def test_upload_single_file(app_client: TestClient, agent_with_dir):
    response = app_client.post(
        "/api/knowledge-bases/homelab/upload",
    ...
    assert "# Nuevo" in app_client.get("/api/knowledge-bases/homelab/files/nuevo.md").text
