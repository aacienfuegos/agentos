...
export function AgentChatPanel({
  ...
  return (
    <div className="flex h-full gap-3 min-h-0">
      {/* Sidebar — conversation picker */}
      {sidebarOpen && (
        <div className="w-44 shrink-0 flex flex-col rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/[0.04] shrink-0">
            <span className="text-[10px] font-mono uppercase tracking-widest text-zinc-600">chats</span>
            <button
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <button
              onClick={resetToNew}
              disabled={sending}
              className={`w-full flex items-center gap-2 px-3 py-2 text-left text-[11px] font-mono border-b border-white/[0.03] transition-colors disabled:opacity-30 ${
                conversationId === null
                  ? "bg-amber-400/[0.06] text-amber-400"
                  : "text-zinc-600 hover:bg-white/[0.02] hover:text-zinc-300"
              }`}
            >
              <span className={`w-1 h-1 rounded-full shrink-0 ${conversationId === null ? "bg-amber-400" : "bg-zinc-800"}`} />
              <span>nueva</span>
            </button>
            ...
            {conversations.map((conv) => (
              <button
                ...
                disabled={sending}
                className={`w-full flex flex-col px-3 py-2 text-left border-b border-white/[0.03] last:border-b-0 transition-colors disabled:opacity-50 ${
                  conversationId === conv.id
                    ? "bg-amber-400/[0.06] text-amber-400"
                    : "text-zinc-500 hover:bg-white/[0.02] hover:text-zinc-300"
                }`}
              >
                <span className="text-[11px] font-mono truncate w-full leading-snug">
                  {conv.firstMessage || conv.id.slice(0, 10)}
                </span>
                <span className="text-[10px] font-mono text-zinc-700 mt-0.5">
                  {conv.turnCount}t · {relTime(conv.lastAt)}
                </span>
              </button>
            ))}
          </div>
