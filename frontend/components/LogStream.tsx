...
export function LogStream({ runId, isLive }: { runId: string; isLive: boolean }) {
  ...
  // Auto-expand when run goes live
  useEffect(() => {
  ...
  return (
    <div className="bg-zinc-950 rounded-lg border border-zinc-800 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 bg-zinc-900">
        ...
        >
          ...
          {!expanded && logs.length > 0 && (
            <span className="text-zinc-600">({logs.length} eventos)</span>
          )}
          {connected && isLive && (
      ...
      {expanded && (
        <div className="border-t border-zinc-800 font-mono text-xs p-4 max-h-[400px] overflow-y-auto space-y-1">
          {logs.length === 0 && (
